...
INSERT INTO categories (name, type, icon, is_default) VALUES
  ...
  ('Investment - Mutual Funds', 'expense', 'AccountBalance', TRUE),
  ('Investment - Stocks', 'expense', 'ShowChart', TRUE),
  ('Other Expense', 'expense', 'MoreHoriz', TRUE);

-- Ensure investment categories always exist as DB default categories (idempotent)
...
CREATE POLICY "Users can view own transactions"
  ON income_expense_transactions FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own transactions"
  ON income_expense_transactions FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own transactions"
  ON income_expense_transactions FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own transactions"
  ON income_expense_transactions FOR DELETE
  USING (auth.uid() = user_id);

-- Mutual Fund Transactions
CREATE POLICY "Users can view own fund transactions"
  ON mutual_fund_transactions FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own fund transactions"
  ON mutual_fund_transactions FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own fund transactions"
  ON mutual_fund_transactions FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own fund transactions"
  ON mutual_fund_transactions FOR DELETE
  USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view own favorites"
  ON favorite_stocks FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own favorites"
  ON favorite_stocks FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can delete own favorites"
  ON favorite_stocks FOR DELETE
  USING (auth.uid() = user_id);
...
CREATE TRIGGER update_ie_updated_at
  BEFORE UPDATE ON income_expense_transactions
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_mf_updated_at
  BEFORE UPDATE ON mutual_fund_transactions
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_st_updated_at
